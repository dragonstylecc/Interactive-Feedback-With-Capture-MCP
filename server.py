...
Cursor has a known bug where `CallMcpTool` occasionally reports "Tool not found" even though the MCP server is running. If this happens:

1. Retry the `interactive_feedback` call up to **2 times** before giving up.
2. If all retries fail, fall back to the built-in `AskQuestion` tool.
